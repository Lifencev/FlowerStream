...
def init_db():
    ...
    with app.app_context():
        ...
        # Створюємо таблицю products (товари/квіти) - НОВА ТАБЛИЦЯ
        cursor.execute('''
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS cart_items (
                ...
                flower_id INTEGER NOT NULL, -- Змінено на flower_id, але посилається на products.id
                quantity INTEGER NOT NULL,
                UNIQUE(user_id, flower_id), 
                FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE,
                FOREIGN KEY (flower_id) REFERENCES products (id) ON DELETE CASCADE -- Посилання на таблицю products
            )
        ''')
